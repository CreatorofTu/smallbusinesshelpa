...
RECIPE_ENCRYPTION_KEY=<from .env.local>
```
...
- **RECIPE_ENCRYPTION_KEY** — the AES-256-GCM key that encrypts recipe data at rest (`api/_recipe-crypto.js`; used by `save-profile.js` on write and `generate-directive.js` on read — see the 2026-07-23 founder-directed recipe reversal in those files' headers). Must be a **64-character hex string (32 bytes)**: generate with `openssl rand -hex 32`, add to `.env.local`, and set it in Vercel — same lifecycle as `SESSION_SECRET`. Never commit it, never put it in client-side code. **Fail-loud, not fail-open:** if it's unset, any save that carries recipe text (the last onboarding step) errors rather than silently storing plaintext — so set this BEFORE deploying the recipe feature. **Rotating it makes every already-stored recipe (and every encrypted recipe change-diff) permanently unreadable** — decryption fails closed and the app treats those recipes as absent; owners would need to re-enter them. There is no re-encryption/migration tool. Treat rotation as a last resort, not routine hygiene.
